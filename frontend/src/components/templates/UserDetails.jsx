...
import { EyeOff, Search, IndianRupee } from "lucide-react";
...
export default function UserDetails() {
  ...
  const getBalance = async () => {
    ...
    try {
      const response = await accountAxios.get("/account/balance");
  ...
  const getUsers = async () => {
    try {
      const response = await userAxios.get("/user/bulk", {
        params: { filter: query },
      });
      ...
      <div className="bg-white shadow rounded-lg p-4">
        ...
        {users.length > 0 && (query.length > 0 || isAllUsers) && (
          <div className="space-y-3">
            {users
              .filter((data) => (user ? data.username !== user.username : true))
              .map((data) => (
                <div
                    ...
                    <button
                      onClick={() => {
                        const obj = {
                          ...
                          payeeId: data.id || data._id,
                          payorUsername: user.username,
                        };
                        setOutletDetails(obj);
                        navigate("/dashboard/send", { state: obj });
                      }}
