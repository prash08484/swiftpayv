...
import { transactionsAxios } from "../../utils/axios";

const SendMoney = () => {
  const navigate = useNavigate();
  const { state: transferDetail } = useLocation();
  ...
  const transfer = async () => {
    if (!transferDetail || !transferDetail.payeeId) {
      toast.error("Recipient details not available");
      return;
    }
    ...
    try {
      await transactionsAxios.post("/transactions/transfer", {
        toUserId: transferDetail.payeeId,
        amount,
      });
      setTransferSuccessful(true);
    } catch (error) {
      toast.error(error.response?.data?.message || error.response?.data?.error || "Transfer failed", {
        position: "bottom-center",
      });
    } finally {
      setLoading(false);
  ...
  return (
    transferDetail && (
      <div className="fixed z-[100] top-0 left-0 w-screen h-screen flex flex-col items-center justify-center bg-[rgba(0,0,0,.8)]">
        <Link onClick={() => navigate(-1)} className="ri-close-line mr-5 absolute text-xl text-white right-5 top-5 hover:scale-110">
          Close
        </Link>
          ...
          {!transferSuccessful ? (
            <div className="flex flex-col gap-[3rem]">
              <div className="flex flex-col items-center justify-center h-[6rem] text-3xl 2xl:h-[10rem] 2xl:text-5xl font-bold pt-[4rem] font-quicksand mb-0">
                <h1>Send Money</h1>
                <img className="h-[7rem] 2xl:h-[10rem]" src="../assets/sendMoney2.jpg" alt="" />
              </div>
              ...
              <div className="px-10 py-0 2xl:py-10">
                <div className="flex gap-3 items-center mb-1 2xl:mb-3">
                  <span className="flex items-center justify-center w-11 h-11 rounded-full bg-[#21C55D] text-xl font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-400 font-quicksand 2xl:text-2xl">
                    {transferDetail.payeeFirstName?.split("")[0] || "U"}
                  </span>
                ...
                <div className="flex flex-col gap-1 2xl:gap-2">
                  ...
                  >
                    {loading ? (
                      <img className="w-full h-6 animate-spin ease-linear mb-1" src="../assets/loading.svg" alt="Loading icon" />
                    ) : (
                      "Initiate transfer"
                    )}
                  </button>
            ...
            <div className="w-full h-full flex flex-col items-center justify-center gap-2">
              <div className="flex flex-col items-center justify-center gap-[3rem]">
                ...
                <h1 className="font-bold text-xl font-quicksand">Transfer Successful</h1>
              </div>
              <span className="text-[0.8rem] text-gray-500">Redirecting in {timer}</span>
            </div>
          )}
        </div>
      </div>
